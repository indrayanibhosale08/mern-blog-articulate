...
const MyBlogsPage = () => {
    ...
    const EmptyState = () => (
        <div className="text-center py-20 px-8 bg-gray-800 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold text-white mb-2">No Articles Yet</h2>
            <p className="text-gray-400 mb-6 max-w-md mx-auto">Click the button to start sharing your ideas with the world.</p>
            <Link to="/dashboard/create-blog" className="inline-block bg-blue-600 text-white font-bold px-8 py-3 rounded-md hover:bg-blue-700 transition-opacity shadow-lg">
                Create Your First Article
            </Link>
        </div>
    );
        ...
        <div className="p-4 sm:p-8 flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                    <h1 className="text-3xl font-bold text-white mb-4 sm:mb-0">Your Blogs</h1>
                    <Link to="/dashboard/create-blog" className="bg-blue-600 text-white font-semibold px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">
                        Create New
                    </Link>
                </div>
                            ...
                            <tbody className="divide-y divide-gray-700">
                                {myBlogs.map((blog) => (
                                    <tr key={blog._id} className="hover:bg-gray-700/50">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm font-semibold text-white">{blog.title}</div>
                                            <div className="text-xs text-gray-400 max-w-xs truncate">{blog.subtitle}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{blog.category}</td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-900 text-green-300">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="relative inline-block text-left" ref={openMenuId === blog._id ? menuRef : null}>
                                                <button onClick={() => setOpenMenuId(openMenuId === blog._id ? null : blog._id)} className="text-gray-400 hover:text-white p-1 rounded-full">
                                                    <DotsVerticalIcon />
                                                </button>
                                                {openMenuId === blog._id && (
                                                    <div className="origin-top-right absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                                                        <div className="py-1">
                                                            <button onClick={() => navigate(`/dashboard/edit-blog/${blog._id}`)} className="w-full text-left px-4 py-2 text-sm text-gray-800 hover:bg-gray-100">
                                                                Edit Article
                                                            </button>
